...
func (client *Client) CreateVIPNode(nodeConfiguration NewVIPNodeConfiguration) (nodeID string, err error) {
	...
	request, err := client.newRequestV26(requestURI, http.MethodPost, &nodeConfiguration)
	responseBody, statusCode, err := client.executeRequest(request)
	if err != nil {
		return "", err
	}
...
func (client *Client) EditVIPNode(id string, nodeConfiguration EditVIPNodeConfiguration) error {
	...
	request, err := client.newRequestV26(requestURI, http.MethodPost, editNodeConfiguration)
	responseBody, statusCode, err := client.executeRequest(request)
	if err != nil {
		return err
	}
...
func (client *Client) DeleteVIPNode(id string) (err error) {
	...
	request, err := client.newRequestV26(requestURI, http.MethodPost, &deleteVIPNode{id})
	responseBody, statusCode, err := client.executeRequest(request)
	if err != nil {
		return err
	}
